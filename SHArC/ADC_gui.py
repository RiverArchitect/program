...
try:
    import Condition_sharea as cSH
...
class ADCgui(object):
    def __init__(self, master, unit, fish_applied, *args):
        ...
        # Set Condition
        self.l_0 = tk.Label(top, text="1) Select a Condition")
        ...
        self.b_sc = tk.Button(top, width=self.col_0_width - 5, fg="firebrick3", bg="white", text="Select",
        ...
        self.l_1 = tk.Label(top, text="2) Generate Ambiance Duration Curves")
        ...
        self.b_q_inp = tk.Button(top, width=self.col_0_width * 2, fg="firebrick3", bg="white",
                                 text="Select input SHArea Series",
                                 command=lambda: self.select_input_shrea_xlxs())
        ...
        self.b_q_dur = tk.Button(top, width=self.col_0_width * 2, fg="firebrick3", bg="white",
                                 text="Make Ambiance duration curve(s)", command=lambda: self.make_ambiance_curve())
    ...
    def make_ambiance_curve(self):
        condition4input = cSH.ConditionCreator(self.dir2condition_act)
        if not self.shrea_series_xlsx:
            self.select_input_shrea_xlxs()

        flow_duration_xlsx = condition4input.create_ambiance_duration_table(self.shrea_series_xlsx,
                                                                        self.eco_flow_type_applied)
        try:
            if not condition4input.error:
                ...
                self.b_q_dur.config(fg="firebrick3", text="Re-try ambiance duration creation")
        except:
            pass
    ...
    def scan_condition(self):
        ...
        if os.path.isfile(self.dir2condition_act + "\\flow_definitions.xlsx"):
            ...
            run = True
        if run:
            condition4flows = cSH.ConditionCreator(self.dir2condition_act)
    ...
    def select_input_shrea_xlxs(self):
        self.shrea_series_xlsx = askopenfilename(initialdir=config.dir2sh + "\\SHArea",
                                                title="Select SHArea workbook (xlsx)",
                                                filetypes=[("Workbooks", "*.xlsx")], parent=self.top)
        b_text = str(self.shrea_series_xlsx)
        if b_text.__len__() > 50:
            b_text = "... \\" + str(self.shrea_series_xlsx).split("/")[-1].split("\\")[-1]
