...
class ConditionCreator:
    ...
    def create_ambiance_duration_table(self, input_shrea_xlsx, aquatic_ambiance):
        try:
            condition = os.path.basename(self.dir2condition)
            flows = cFl.AmbianceProcessor(input_shrea_xlsx)
            output_xlsx = flows.make_ambiance_duration(condition)
            flows.make_condition_flow2d_duration(condition)
            flows.make_disc_freq(condition)
            return output_xlsx
        except:
    ...
    def create_sub_condition(self, dir2src_condition, dir2bound):
        ...
        try:
            ...
            for f_xlsx in all_flow_files:
                try:
                    copyfile(config.dir2flows + src_condition + "\\" + f_xlsx,
                             config.dir2flows + self.condition + "\\" + f_xlsx)
                except:
                    pass
        except:
            self.logger.info(
                " *** Could not find FLOW DEFINITIONS for %s. Use ANALYZE FLOWS for new sub-condition." % dir2src_condition)
        try:
    ...
    def transfer_rasters_from_folder(self, folder_dir, type_id, string_container):
        ...
        self.logger.info(
            " > Getting {0} Rasters containing {1} from {2}.".format(str(type_id), string_container, str(folder_dir)))
            ...
            for filename in filenames:
                if filename.endswith(".tif"):
                    try:
                        ...
                        cells_match, coords_match, corners_match = self.check_alignment_pair(snap_raster,
                                                                                             misaligned_raster)
                        ...
                        if not (cells_match and coords_match and corners_match):
                            ...
                            if filename.startswith("u") or filename.startswith("va"):
                                try:
                                    ...
                                    self.logger.info("WARNING: Invalid alignment Raster name (%s)." % str(
                                        os.path.join(root, filename.replace('u', 'h').replace('va', 'h'))))
                                    continue
